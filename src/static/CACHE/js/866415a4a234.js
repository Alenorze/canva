    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (current_input.type == 'radio' && $(current_input).is(':checked')) {
        ...
        result_container.append(
          "<li class='js-item-remove'><span>" + name + ":</span> " + text + "</li>"
        );
        ...
        var name = current_input.name.split('__')[0]
        result_container.append(
          "<li class='js-item-remove'><span>" + name + ":</span> " + $(current_input).val() + "</li>"
        );
      } else if (current_input.type == 'checkbox' && $(current_input).is(':checked')) {
        var text = $(current_input).parent().text();
        result_container.append(
          "<li class='js-item-remove'><span>" + text + ":</span> " + 'Yes' + "</li>"
        );
      };
    };
...
window.after_callbacks = {
		'.js-location-form':function(frm, html){
			if(frm.data('id')){
				var loc = $('.js-location-entry[data-id='+frm.data('id')+']'),
					addr = frm.find('#js-location-popup').val();

				if(loc.is('.js-location-entry:first'))
					$('#js-location').val(addr);
			}
		...
		success:function(data){
			if(t.is('.js-verify-phone')){
				...
				var msg = clearTwilioStr(
					...
					var msg = $(data['html'])
						.children('.errorlist li').text();
				}
				if(typeof msg == 'undefined'){
					li = $(data['html']).children('.errorlist li')
					msg = li.length ? li.text() : $(data['html']).eq(0).text();
				}
				resp.find('.js-verify-message').text(
					msg
				);
				if(data['success']){
