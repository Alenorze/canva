...
;    var canvas = document.getElementById("myCanvas");
    ...
    $(document).ready(function () {
    	...
    	$(window).on('load', function () {
    		$.get('http://localhost:3000/images', function (data) {
    			dataImg = data;
    			arrayDoc = data;
    			let UlElement;
    			let liElement;
    			...
    			jQuery.each(data, function (a, value) {
    				...
    				jQuery.each(value, function (index, item) {
    					liElement = `<li value="` + index + `" class="rightBlockImg" onclick="$(this).scrollCanvas()"><img src="` + item.src + `"></li>`;
    		...
    		$.get('http://localhost:3000/forms', function (data) {
    			jQuery.each(data, function (i, val) {
    				jQuery.each(val, function (index, item) {
    					arrayUser[index] = [item];
    					arrInfo.push(item);
    					var liElement = `<li value="` + index + `" class='selectDoc-li' onclick="$(this).defineColor()">
                             <i class="fa fa-user ` + arrColorIconListUsers[index] + `" aria-hidden="true"></i>` + item.name + `</li>`
    	...
    	$.fn.getAttrUl = function () {
    		...
    		let totalHeight = 0;
    		jQuery.each(globalArray[0][0][0].img[indexDoc], function (a, val) {
    			image = new Image();
    			image.src = val.src;
    	...
    	$('#btnSend').on('click', function () {
    		...
    		$.ajax({
    			...
    			type: 'POST',
    			crossDomain: true,
    			data: JSON.stringify(globalArray),
...
;$(document).ready(function () {
	...
	//move form
	$('.formBlock-wrapper').on('click', function (e) {
		$(this).sortable({
	...
	$('.btnNext').on('click', function (e) {
		...
		if (disableLink == 1) {
			$.ajax({
				...
				type: 'POST',
				crossDomain: true,
				data: JSON.stringify(arrRecipients),
				contentType: 'application/json; charset=utf-8',
				dataType: 'json',
				async: false,
	...
	$(window).scroll(function () {
		if ($(this).scrollTop() > 66 && rightBlockForm.hasClass("default")) {
			rightBlockForm.removeClass("default").addClass("fixed");
		} else if ($(this).scrollTop() <= 66 && rightBlockForm.hasClass("fixed")) {
...
;var canvas = document.getElementById("canvasUser");
var ctx = canvas.getContext("2d");

$(document).ready(function () {

	var firstVisit = false;
	//set user index for working with images 
	var indexUser = 0;
	//define what document load
	let indexImages = 0;
    //we sew clicks on pictures
	var index = 1;
	var textSignature = $('#textSignature');
    var canvasBlock = $('.canvasBlock');
	var canvasUser = $('#canvasUser');
	var popUpDocumentWrapper = $('.popUpDocument-wrapper');
	var arrObj = [];
	var arrClickObj = [];
	...
	var sumImg;
	var globalArray = [];
	var attrDate = "Times New Roman";
	var signaturaBlock = $('.signaturaBlock');
	var signature;
	//know the time
	var date = new Date();
    var indexClick = 1;
	var time = date.getDate() + '/' + date.getMonth() + '/' + date.getFullYear() + ' ' + date.getHours() + ':' + (date.getMinutes() < 10 ? "0" : "") + date.getMinutes() + ':' +( date.getSeconds() < 10 ? "0" : "" ) + date.getSeconds();
	
    //array icons
	var arrIconsJs = [0xf044, 0xf040, 0xf073, 0xf007, 0xf0e0, 0xf1ad, 0xf0f6, 0xf14b, 0xf046];
	//array fonts
	var arrClassFont = ['Charmonman', 'Cookie', 'Great Vibes', 'Homemade Apple', 'Sacramento']
    //textarea block
	var addTextarea = `<div class="textareaBlock">
                   <i class="fas fa-arrows-alt faArrowsAlt"></i>
                     <textarea class="addText"></textarea>
                    <div class="fontFamilys">
                        <div class="fontFamily roboto" data-font-family='roboto' title='Roboto' onclick="$(this).addAttr()">Aa</div>
                        <div class="fontFamily opensans" data-font-family='opensans' title='Open Sans' onclick="$(this).addAttr()">Aa</div>
                        <div class="fontFamily soursesanspro" data-font-family='soursesanspro' title='Source Sans Pro' onclick="$(this).addAttr()">Aa</div>
                        <div class="fontFamily ubuntu" data-font-family='ubuntu' title='Ubuntu' onclick="$(this).addAttr()">Aa</div>
                    </div>
                    <button class='btnHideTextarea' onclick="$(this).removeTextarea()">add text</button>
                </div>`;
    //load data
	$(window).on('load', function () {
		$.get('http://localhost:3000/agent', function (data) {
			globalArray = data[0];
	});
		if (firstVisit == true) {
			$('.infoBlock').css('display', 'block');
		}
		$.get('http://localhost:3000/images', function (dataImg) {
			jQuery.each(dataImg, function (a, value) {
				jQuery.each(value, function (i, val) {
					var img = '<img src="'+val.src+'" alt="">';
					$('.popUpDocument').append(img);		
				});
			});
		});	
	});
	
	//show popup documents
	$('.btnReview').on('click', function () {
		popUpDocumentWrapper.fadeIn(1000);
	});
    //hide popup documents
	$('.faTimesCirclePopUp').on('click', function () {
		popUpDocumentWrapper.fadeOut(1000);
	});
	...
	function startSigning() {
		jQuery.each(globalArray[indexImages], function (i, val) {
		...
		arrayObj = [];
		drawObj = [];
		if(globalArray[0].length > indexImages){
			
			for(var c = 0;c < (indexUser + 1);c++){
				arrayObj[c] = globalArray[indexImages][c];
				jQuery.each(arrayObj[c], function (a, item) {
				drawObj.push(item);
			});		
		}
			
		jQuery.each(globalArray[indexImages][indexUser], function (a, value) {
				arrClickObj.push(value);
			});
		}else{
			finishSig();
			return false;
		...
		jQuery.each(arrRects, function (b, val) {
			arrObj.push(val)
			jQuery.each(arrObj[0].img[indexImages], function (i, obj) {
				image = new Image();
				image.src = obj.src;
				imgHeight = image.naturalHeight;
				imgWidth = image.naturalWidth;
			});
			canvasUser.attr('height', imgHeight * arrObj[0].img[indexImages].length).attr('width', imgWidth);
		});
		draw();
		scrollRect();
	};
	...
	//add class font family
	$.fn.addAttr = function () {
		var addText = $('.addText');
		var fontFamily = $(this).attr('data-font-family');
		addText.removeClass().addClass(fontFamily + ' addText');
	}
	
	function addSignatureText(thisObj, signature) {
		thisObj.parent().parent().fadeOut(500);
		signature.famely = textSignature.attr('data-font');
		signature.add = true;
		draw();
		scrollRect();
	};
	...
	function draw() {
		...
		jQuery.each(arrObj[0].img[indexImages], function (i, obj) {
			image.src = obj.src;
			ctx.drawImage(image, 0, imgHeight * i, imgWidth, imgHeight);
		});
		ctx.fillStyle = "rgba(255, 235, 0, 0)";
		rect(0, 0, WIDTH, HEIGHT);
		for (var j = 0; j < drawObj.length; j++) {
			let r = drawObj[j];
			var positionTop = r.y;
			var positionLeft = r.x;
			var text = r.text;
			var positionTopIcon = positionTop + 15
			...
			if (r.signature.add == true) {
				ctx.font = '35px ' + r.signature.famely;
				ctx.fillText(r.signature.textsign, posIconX, posIconY);
			}
			if (words.length) {
				for (var n = 0; n < countWords; n++) {
					...
					if (words.length < 3) {
						console.log()
						line = testLine;
						ctx.fillText(line, posX + 40, positionTopIcon + marginTop);
					} else {

						if (testWidth > r.width) {
							var dd = positionTop + marginT;
							ctx.fillText(line, posX, dd);
							line = words[n] + " ";
							marginT += lineHeight;
						} else {
							line = testLine;
							ctx.fillText(line, posX, positionTop + marginT);
						}
	...
	//add text block
	function addText() {
		$('.canvasWrapper').append(addTextarea);
		$('.textareaBlock').draggable({
			handle: ".faArrowsAlt"
		});
	}
 	
	$('#canvasUser').on('click', function (e) {
		// tell the browser we're handling this mouse event
		e.preventDefault();
		e.stopPropagation();
		// get the current mouse position
		...
		var my = e.offsetY;
		// test each rect to see if mouse is inside
		for (var i = 1; i < arrClickObj.length; i++) {
			var r = arrClickObj[indexClick];
			if (mx > r.x && mx < r.x + r.width && my > r.y && my < r.y + r.height) {
				indexClick++
				if (r.text == 'DATE SIGNED') {
					r.text = time;
					addBlockNode(r.node);
				}
				if (r.text == 'SIGNATURE') {
					...
					$('#btnAddSignatureText').click(function () {
						r.text = '';
						r.fill = 'transparent';
						r.icons = '';
						draw();
						addSignatureText($(this), signature);
					});
					addSignatureBlock(r.x, r.y, signature);
					return false;
				}
				if (r.text == 'CHECKBOX') {
					r.icons = r.icons;
				...
				if (r.text == 'TEXT') {
					addBlockNode(r.node);
					addText();
					$('.btnHideTextarea:last').on('click', function () {
						var textImg = $('.addText:last').val();
						r.text = textImg;
						r.fill = 'transparent';
						r.icons = '';
						draw();
	...
	//hide block textarea
	$.fn.removeTextarea = function () {
		$(this).parent().fadeOut(500);
	}
	//add signature block
	function addSignatureBlock(x, y, signature) {
		signaturaBlock.children().children('#textSignature').text(signature.textsign);
		signaturaBlock.fadeIn(500);
		signaturaBlock.draggable({
			handle: ".faArrowsAlt"
		});
	}
	...
	//scroll canvas
	function scrollRect() {
		if (index < arrClickObj.length) {
			ctx.lineWidth = 3;
			ctx.strokeStyle = '#fff500';
			ctx.strokeRect(arrClickObj[index].x, arrClickObj[index].y, arrClickObj[index].width, arrClickObj[index].height);
			canvasBlock.animate({
				scrollTop: arrClickObj[index].y - 50,
				scrollLeft: arrClickObj[index].x - 50
			}, 1000);
			index++
			return false;
		} else {
	...
	//hide block node
	$('#btnHideNode').on('click', function () {
		hideNode();	
	});
	//hide block node
	function hideNode(){
		$('.nodeBlock').fadeOut(500);	
	} 
	
	//show block node
	function addBlockNode(textNode) {
		if (textNode != '') {
			var nodeBlock = $('.nodeBlock')
			nodeBlock.fadeIn(500);
			nodeBlock.children('.nodeBlockText').text(textNode);
		}
	...
	$('.family').on('click', function () {
		var data = $(this).attr('data-font-family');
		var fontFamely = $(this).attr('title');
		textSignature.removeClass();
		textSignature.addClass(data);
		textSignature.attr('data-font', fontFamely);
	});
	...
	//set the width of the block with the text
	function widthText(text, maxWidth, posX, posY) {
		var lineHeight = 20;
		var marginTop = 20;
		var positionTop = posY;
		var words = text.split(" ");
		var countWords = words.length;
		var line = "";
		ctx.font = "12px Arial";
		for (var n = 0; n < countWords; n++) {
			...
			if (testWidth > maxWidth) {
				ctx.fillText(line, posX, positionTop + marginTop);
				line = words[n] + " ";
		...
		positionTop = 0;
 				...
 				//open images
//            document.getElementsByClassName('btnFinish')[0].onclick = function(){
//                var canvas = document.getElementById("canvasUser")
